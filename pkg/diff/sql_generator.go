...
// 3. Generate the SQL required for the deleted items (DELETES), the new items, and the altered items.
...
//
// SQL generation for the resulting diff is handled by sqlGenerator and sqlVertexGenerator implementations.
// Both generate the SQL required to add, delete, or alter a schema object. When altering an object, the generator
// receives the diff produced by diffLists. A sqlVertexGenerator also defines dependencies on other schema objects.
...
func buildTableDiff(oldTable, newTable schema.Table) (diff tableDiff, requiresRecreation bool, err error) {
	...
	psg, err := newPolicySQLVertexGenerator(&oldTable, newTable)
	if err != nil {
...
func (s schemaSQLGenerator) Alter(diff schemaDiff) ([]Statement, error) {
	...
	attachPartitionsPartialGraph, err := generatePartialGraph(
		attachPartitionGenerator, diff.tableDiffs,
	)
	if err != nil {
	...
	sequenceOwnershipGenerator := &sequenceOwnershipSQLVertexGenerator{}
	...
	functionGenerator := &functionSQLVertexGenerator{}
...
func buildIndexesByTableNameMap(indexes []schema.Index) map[string][]schema.Index {
	output := make(map[string][]schema.Index)
...
func buildChildrenByPartitionedIndexNameMap(indexes []schema.Index) map[string][]schema.Index {
	...
	childrenByDirectParentIdxName := make(map[string][]schema.Index)
	for _, idx := range indexes {
		parentIdxName := ""
	...
	rootsByChildIdxName := make(map[string]schema.Index)
	// We need to calculate this level by level. We will start with the roots.
	currentNodes := childrenByDirectParentIdxName[""]
	...
	allChildrenByRootIndexName := make(map[string][]schema.Index)
	for _, idx := range indexes {
		if idx.ParentIdx == nil {
...
func (t *tableSQLVertexGenerator) Add(table schema.Table) (partialSQLGraph, error) {
	statements, err := t.addStatements(table)
	if err != nil {
		return partialSQLGraph{}, fmt.Errorf("generating sql: %w", err)
	}
	return buildPartialSQLGraph(
		buildTableVertexId(table.SchemaQualifiedName, diffTypeAddAlter),
		sqlPrioritySooner,
		statements,
		t.getAddAlterDependencies(table),
	), nil
}

func (t *tableSQLVertexGenerator) addStatements(table schema.Table) ([]Statement, error) {
	if table.IsPartition() {
		...
		table.CheckConstraints = append(table.CheckConstraints,
			t.tablesInNewSchemaByName[table.ParentTable.GetName()].CheckConstraints...)
	}
	...
	if table.IsPartitioned() {
		fmt.Fprintf(&createTableSb, " PARTITION BY %s", table.PartitionKeyDef)
	}
	...
	for _, checkCon := range table.CheckConstraints {
		addConStmts, err := csg.addStatements(checkCon)
		if err != nil {
	...
	if table.ReplicaIdentity != schema.ReplicaIdentityDefault {
		...
		alterReplicaIdentityStmt, err := alterReplicaIdentityStatement(
			table.SchemaQualifiedName, table.ReplicaIdentity,
		)
		if err != nil {
	...
	for _, policy := range table.Policies {
		if _, err := policyGenerator.addDependencies(policy); err != nil {
			return nil, fmt.Errorf("getting dependencies for policy %s: %w", policy.EscapedName, err)
		}
		addPolicyStmts, err := policyGenerator.addStatements(policy)
		if err != nil {
	...
	for _, privilege := range table.Privileges {
		// Remove hazards from statements since the table is brand new
		stmts = append(stmts, stripMigrationHazards(
			privilegeGenerator.addStatements(privilege)...,
		)...)
	}
...
func (t *tableSQLVertexGenerator) Delete(table schema.Table) (partialSQLGraph, error) {
	statements, err := t.deleteStatements(table)
	if err != nil {
		return partialSQLGraph{}, fmt.Errorf("generating sql: %w", err)
	}
	return buildPartialSQLGraph(
		buildTableVertexId(table.SchemaQualifiedName, diffTypeDelete),
		sqlPriorityLater,
		statements,
		t.getDeleteDependencies(table),
	), nil
}
...
func (t *tableSQLVertexGenerator) Alter(diff tableDiff) (partialSQLGraph, error) {
	statements, err := t.alterStatements(diff)
	if err != nil {
		return partialSQLGraph{}, fmt.Errorf("generating sql: %w", err)
	}
	return buildPartialSQLGraph(
		buildTableVertexId(diff.new.SchemaQualifiedName, diffTypeAddAlter),
		sqlPrioritySooner,
		statements,
		t.getAddAlterDependencies(diff.new),
	), nil
}

func (t *tableSQLVertexGenerator) alterStatements(diff tableDiff) ([]Statement, error) {
	...
	if diff.old.ReplicaIdentity != diff.new.ReplicaIdentity {
		alterReplicaIdentityStmt, err := alterReplicaIdentityStatement(
			diff.new.SchemaQualifiedName, diff.new.ReplicaIdentity,
		)
		if err != nil {
			return nil, fmt.Errorf("building replica identity statement: %w", err)
		}
	...
	// outage while RLS is being enabled
	if diff.new.RLSEnabled && !diff.old.RLSEnabled {
...
func (t *tableSQLVertexGenerator) alterBaseTable(diff tableDiff) ([]Statement, error) {
	...
	for _, colDiff := range getDangerousNotNullAlters(diff.columnsDiff.alters,
		diff.new.CheckConstraints, diff.old.CheckConstraints) {
	...
	if err != nil {
		return nil, fmt.Errorf("resolving column diff: %w", err)
	}
	partialGraph = concatPartialGraphs(partialGraph, columnsPartialGraph)

	checkConGenerator := &checkConstraintSQLVertexGenerator{
	...
	for _, tempCC := range tempCCs {
		dropTempCCStatements, err := checkConGenerator.deleteStatements(tempCC)
		if err != nil {
			return nil, fmt.Errorf("deleting temp check constraint: %w", err)
		}
		dropTempCCs = append(dropTempCCs, dropTempCCStatements...)
	}

	policyGenerator, err := newPolicySQLVertexGenerator(&diff.old, diff.new)
	if err != nil {
		return nil, fmt.Errorf("creating policy sql vertex generator: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("getting ordered statements from table graph: %w", err)
	}
...
func (t *tableSQLVertexGenerator) alterPartition(diff tableDiff) ([]Statement, error) {
	...
	for _, colDiff := range diff.columnsDiff.alters {
		...
		if parentCol, ok := alteredParentColumnsByName[colDiff.new.Name]; ok {
			if colDiff.new.IsNullable == parentCol.new.IsNullable &&
				parentCol.new.IsNullable != parentCol.old.IsNullable {
		...
		alterColumnPrefix := fmt.Sprintf("%s ALTER COLUMN %s",
			alterTablePrefix(diff.new.SchemaQualifiedName),
			schema.EscapeIdentifier(colDiff.new.Name))
...
func (t *tableSQLVertexGenerator) getAddAlterDependencies(table schema.Table) []dependency {
	deps := []dependency{
		mustRun(buildTableVertexId(table.SchemaQualifiedName, diffTypeAddAlter)).after(
			buildTableVertexId(table.SchemaQualifiedName, diffTypeDelete),
		),
	}
	...
	if table.ParentTable != nil {
		deps = append(
			deps,
			mustRun(buildTableVertexId(table.SchemaQualifiedName, diffTypeAddAlter)).after(
				buildTableVertexId(*table.ParentTable, diffTypeAddAlter),
			),
		)
	}
	return deps
...
var isNotNullCCRegex = regexp.MustCompile(`^\(*((".*")|(\S*)) IS NOT NULL\)*$`)
...
func buildTempNotNullConstraint(randReader io.Reader, colDiff columnDiff) (schema.CheckConstraint, error) {
	...
	return schema.CheckConstraint{
		...
		Expression: fmt.Sprintf("%s IS NOT NULL",
			schema.EscapeIdentifier(colDiff.new.Name)),
		IsValid:            true,
		IsInheritable:      true,
...
func (t *tableSQLVertexGenerator) getDeleteDependencies(table schema.Table) []dependency {
	...
	if table.ParentTable != nil {
		deps = append(
			deps,
			mustRun(buildTableVertexId(table.SchemaQualifiedName, diffTypeDelete)).after(
				buildTableVertexId(*table.ParentTable, diffTypeDelete),
			),
		)
	}
	return deps
...
func newColumnSQLVertexGenerator(tableName schema.SchemaQualifiedName) *columnSQLVertexGenerator {
	return &columnSQLVertexGenerator{
		tableName: tableName,
	}
}

func (csg *columnSQLVertexGenerator) Add(column schema.Column) (partialSQLGraph, error) {
	statements, err := csg.addStatements(column)
	if err != nil {
		return partialSQLGraph{}, fmt.Errorf("generating sql: %w", err)
	}
	return buildPartialSQLGraph(
		buildColumnVertexId(column.Name, diffTypeAddAlter),
		sqlPrioritySooner,
		statements,
		csg.getAddAlterDependencies(column),
	), nil
}

func (csg *columnSQLVertexGenerator) addStatements(column schema.Column) ([]Statement, error) {
	columnDef, err := buildColumnDefinition(column)
	if err != nil {
		return nil, fmt.Errorf("building column definition: %w", err)
	}
...
func (csg *columnSQLVertexGenerator) Delete(column schema.Column) (partialSQLGraph, error) {
	return buildPartialSQLGraph(
		buildColumnVertexId(column.Name, diffTypeDelete),
		sqlPriorityLater,
		csg.deleteStatements(column),
		nil,
	), nil
}

func (csg *columnSQLVertexGenerator) deleteStatements(column schema.Column) []Statement {
	return []Statement{{
...
func (csg *columnSQLVertexGenerator) Alter(diff columnDiff) (partialSQLGraph, error) {
	statements, err := csg.alterStatements(diff)
	if err != nil {
		return partialSQLGraph{}, fmt.Errorf("generating sql: %w", err)
	}
	return buildPartialSQLGraph(
		buildColumnVertexId(diff.new.Name, diffTypeAddAlter),
		sqlPrioritySooner,
		statements,
		csg.getAddAlterDependencies(diff.new),
	), nil
}

func (csg *columnSQLVertexGenerator) alterStatements(diff columnDiff) ([]Statement, error) {
	...
	if !strings.EqualFold(oldColumn.Type, newColumn.Type) ||
		!strings.EqualFold(oldColumn.Collation.GetFQEscapedName(),
			newColumn.Collation.GetFQEscapedName()) {
		stmts = append(stmts,
			[]Statement{
				...
				{
					DDL: fmt.Sprintf("ANALYZE %s (%s)", csg.tableName.GetFQEscapedName(),
						schema.EscapeIdentifier(newColumn.Name)),
					Hazards: []MigrationHazard{
						{
...
func (csg *columnSQLVertexGenerator) generateTypeTransformationStatement(
	...
	if strings.EqualFold(oldType, "bigint") &&
		...
		return Statement{
			DDL: fmt.Sprintf(
				"%s SET DATA TYPE %s using to_timestamp(%s / 1000)",
...
func (csg *columnSQLVertexGenerator) getAddAlterDependencies(col schema.Column) []dependency {
	return []dependency{
		mustRun(buildColumnVertexId(col.Name, diffTypeDelete)).before(
			buildColumnVertexId(col.Name, diffTypeAddAlter),
		),
	}
}
...
func (rsg *renameConflictingIndexSQLVertexGenerator) Add(index schema.Index) (partialSQLGraph, error) {
	statements, err := rsg.addStatements(index)
	if err != nil {
		return partialSQLGraph{}, fmt.Errorf("generating sql: %w", err)
	}
	return buildPartialSQLGraph(
		buildRenameConflictingIndexVertexId(index.GetSchemaQualifiedName(), diffTypeAddAlter),
		sqlPrioritySooner,
		statements,
		nil,
	), nil
...
func (rsg *renameConflictingIndexSQLVertexGenerator) addStatements(index schema.Index) ([]Statement, error) {
	...
	return []Statement{{
		DDL: fmt.Sprintf("ALTER INDEX %s RENAME TO %s",
			index.GetSchemaQualifiedName().GetFQEscapedName(), schema.EscapeIdentifier(newName)),
	}}, nil
}
...
func (rsg *renameConflictingIndexSQLVertexGenerator) Delete(index schema.Index) (partialSQLGraph, error) {
	return buildPartialSQLGraph(
		buildRenameConflictingIndexVertexId(index.GetSchemaQualifiedName(), diffTypeDelete),
		sqlPriorityLater,
		nil,
		nil,
	), nil
...
func (rsg *renameConflictingIndexSQLVertexGenerator) Alter(diff indexDiff) (partialSQLGraph, error) {
	return buildPartialSQLGraph(
		buildRenameConflictingIndexVertexId(diff.new.GetSchemaQualifiedName(), diffTypeAddAlter),
		sqlPrioritySooner,
		nil,
		nil,
	), nil
...
func (isg *indexSQLVertexGenerator) Add(index schema.Index) (partialSQLGraph, error) {
	statements, err := isg.addStatements(index)
	if err != nil {
		return partialSQLGraph{}, fmt.Errorf("generating sql: %w", err)
	}
	return buildPartialSQLGraph(
		buildIndexVertexId(index.GetSchemaQualifiedName(), diffTypeAddAlter),
		sqlPrioritySooner,
		statements,
		isg.getAddAlterDependencies(index),
	), nil
}

func (isg *indexSQLVertexGenerator) addStatements(index schema.Index) ([]Statement, error) {
...
func (isg *indexSQLVertexGenerator) addIdxStmtsWithHazards(index schema.Index) ([]Statement, error) {
	...
	if isOnPartitionedTable && index.Constraint != nil {
		...
		return []Statement{{
			DDL: fmt.Sprintf("ALTER TABLE ONLY %s ADD CONSTRAINT %s %s",
				index.OwningRelName.GetFQEscapedName(), index.Constraint.EscapedConstraintName, index.Constraint.ConstraintDef),
		}}, nil
	}
...
func (isg *indexSQLVertexGenerator) Delete(index schema.Index) (partialSQLGraph, error) {
	statements, err := isg.deleteStatements(index)
	if err != nil {
		return partialSQLGraph{}, fmt.Errorf("generating sql: %w", err)
	}
	return buildPartialSQLGraph(
		buildIndexVertexId(index.GetSchemaQualifiedName(), diffTypeDelete),
		sqlPriorityLater,
		statements,
		isg.getDeleteDependencies(index),
	), nil
}
...
func (isg *indexSQLVertexGenerator) deleteStatements(index schema.Index) ([]Statement, error) {
	...
	return []Statement{{
		DDL: fmt.Sprintf("DROP INDEX CONCURRENTLY %s",
			indexToDrop.GetSchemaQualifiedName().GetFQEscapedName()),
		Hazards: []MigrationHazard{migrationHazardIndexDroppedQueryPerf},
	}}, nil
}

func (isg *indexSQLVertexGenerator) Alter(diff indexDiff) (partialSQLGraph, error) {
	statements, err := isg.alterStatements(diff)
	if err != nil {
		return partialSQLGraph{}, fmt.Errorf("generating sql: %w", err)
	}
	return buildPartialSQLGraph(
		buildIndexVertexId(diff.new.GetSchemaQualifiedName(), diffTypeAddAlter),
		sqlPrioritySooner,
		statements,
		isg.getAddAlterDependencies(diff.new),
	), nil
}

func (isg *indexSQLVertexGenerator) alterStatements(diff indexDiff) ([]Statement, error) {
	var stmts []Statement
...
func buildAttachIndex(index schema.Index) Statement {
	return Statement{
		DDL: fmt.Sprintf("ALTER INDEX %s ATTACH PARTITION %s",
			index.ParentIdx.GetFQEscapedName(), index.GetSchemaQualifiedName().GetFQEscapedName()),
	}
}
...
func (isg *indexSQLVertexGenerator) getAddAlterDependencies(index schema.Index) []dependency {
	addAlterVertexID := buildIndexVertexId(index.GetSchemaQualifiedName(), diffTypeAddAlter)
	dependencies := []dependency{
		...
		mustRun(addAlterVertexID).after(
			buildRenameConflictingIndexVertexId(index.GetSchemaQualifiedName(), diffTypeAddAlter),
		),
	}
	...
	if index.OwningRelKind == schema.RelKindMaterializedView {
		dependencies = append(dependencies,
			mustRun(addAlterVertexID).after(
				buildMaterializedViewVertexId(index.OwningRelName, diffTypeAddAlter),
			))
	} else {
		dependencies = append(dependencies,
			mustRun(addAlterVertexID).after(
				buildTableVertexId(index.OwningRelName, diffTypeAddAlter),
			))
	}
	...
	if index.ParentIdx != nil {
		...
		dependencies = append(dependencies,
			mustRun(addAlterVertexID).after(
				buildIndexVertexId(*index.ParentIdx, diffTypeAddAlter),
			))
	}
...
func (isg *indexSQLVertexGenerator) getDeleteDependencies(index schema.Index) []dependency {
	deleteVertexID := buildIndexVertexId(index.GetSchemaQualifiedName(), diffTypeDelete)
	dependencies := []dependency{
		// Drop the index after it has been potentially renamed
		mustRun(deleteVertexID).after(
			buildRenameConflictingIndexVertexId(index.GetSchemaQualifiedName(), diffTypeAddAlter),
		),
	}
	...
	if index.OwningRelKind == schema.RelKindMaterializedView {
		dependencies = append(dependencies,
			mustRun(deleteVertexID).after(
				buildMaterializedViewVertexId(index.OwningRelName, diffTypeDelete),
			))
	} else {
		dependencies = append(dependencies,
			mustRun(deleteVertexID).after(
				buildTableVertexId(index.OwningRelName, diffTypeDelete),
			))
	}
	...
	if index.ParentIdx != nil {
		...
		dependencies = append(dependencies,
			mustRun(deleteVertexID).after(
				buildIndexVertexId(*index.ParentIdx, diffTypeDelete),
			))
	}
...
func (isg *indexSQLVertexGenerator) addDepsOnRelationAddAlterIfNecessary(index schema.Index) []dependency {
	...
	if parentTable.ParentTable != nil {
		...
		addAlterColumnDeps = append(
			addAlterColumnDeps,
			mustRun(deleteVertexID).before(
				buildTableVertexId(*parentTable.ParentTable, diffTypeAddAlter),
			),
		)
	}
...
func (csg *checkConstraintSQLVertexGenerator) Add(con schema.CheckConstraint) (partialSQLGraph, error) {
	statements, err := csg.addStatements(con)
	if err != nil {
		return partialSQLGraph{}, fmt.Errorf("generating sql: %w", err)
	}
	dependencies, err := csg.getAddAlterDependencies(con)
	if err != nil {
		return partialSQLGraph{}, fmt.Errorf("getting dependencies: %w", err)
	}
	return buildPartialSQLGraph(
		buildCheckConstraintVertexId(con, diffTypeAddAlter),
		sqlPrioritySooner,
		statements,
		dependencies,
	), nil
...
func (csg *checkConstraintSQLVertexGenerator) addStatements(con schema.CheckConstraint) ([]Statement, error) {
...
func (csg *checkConstraintSQLVertexGenerator) Delete(con schema.CheckConstraint) (partialSQLGraph, error) {
	statements, err := csg.deleteStatements(con)
	if err != nil {
		return partialSQLGraph{}, fmt.Errorf("generating sql: %w", err)
	}
	dependencies, err := csg.getDeleteDependencies(con)
	if err != nil {
		return partialSQLGraph{}, fmt.Errorf("getting dependencies: %w", err)
	}
	return buildPartialSQLGraph(
		buildCheckConstraintVertexId(con, diffTypeDelete),
		sqlPriorityLater,
		statements,
		dependencies,
	), nil
...
func (csg *checkConstraintSQLVertexGenerator) deleteStatements(con schema.CheckConstraint) ([]Statement, error) {
...
func (csg *checkConstraintSQLVertexGenerator) Alter(diff checkConstraintDiff) (partialSQLGraph, error) {
	statements, err := csg.alterStatements(diff)
	if err != nil {
		return partialSQLGraph{}, fmt.Errorf("generating sql: %w", err)
	}
	dependencies, err := csg.getAddAlterDependencies(diff.new)
	if err != nil {
		return partialSQLGraph{}, fmt.Errorf("getting dependencies: %w", err)
	}
	return buildPartialSQLGraph(
		buildCheckConstraintVertexId(diff.new, diffTypeAddAlter),
		sqlPrioritySooner,
		statements,
		dependencies,
	), nil
...
func (csg *checkConstraintSQLVertexGenerator) alterStatements(diff checkConstraintDiff) ([]Statement, error) {
	oldCopy := diff.old

	var stmts []Statement
	if !diff.old.IsValid && diff.new.IsValid {
		stmts = append(stmts, validateConstraintStatement(csg.tableName,
	...
	if !cmp.Equal(oldCopy, newCopy) {
		...
		return nil, fmt.Errorf("altering check constraint to resolve the following diff %s: %w",
			cmp.Diff(oldCopy, diff.new), ErrNotImplemented)
...
func buildCheckConstraintVertexId(con schema.CheckConstraint, diffType diffType) sqlVertexId {
	return buildSchemaObjVertexId("checkconstraint", con.Name, diffType)
}

func (csg *checkConstraintSQLVertexGenerator) getAddAlterDependencies(con schema.CheckConstraint) ([]dependency, error) {
	addAlterVertexID := buildCheckConstraintVertexId(con, diffTypeAddAlter)
	deps := []dependency{
		mustRun(buildCheckConstraintVertexId(con, diffTypeDelete)).before(
			addAlterVertexID,
		),
	}
	...
	if isOnValidNotNullPreExistingColumn {
		...
		// the column alter.
		deps = append(deps, mustRun(addAlterVertexID).before(
			buildColumnVertexId(targetColumns[0].Name, diffTypeAddAlter),
		))
	} else {
		for _, tc := range targetColumns {
			deps = append(deps, mustRun(addAlterVertexID).after(
...
func (csg *checkConstraintSQLVertexGenerator) getDeleteDependencies(con schema.CheckConstraint) ([]dependency, error) {
	var deps []dependency
	deleteVertexID := buildCheckConstraintVertexId(con, diffTypeDelete)
	...
	if isValidNotNullCC(con) {
		...
		if _, ok := csg.deletedColumnsByName[tc.Name]; ok {
			...
			deps = append(deps, mustRun(deleteVertexID).before(
				buildColumnVertexId(tc.Name, diffTypeDelete),
			))
		} else {
			...
			deps = append(deps, mustRun(deleteVertexID).after(
				buildColumnVertexId(tc.Name, diffTypeAddAlter),
			))
		}
	} else {
...
) *attachPartitionSQLVertexGenerator {
...
func (a *attachPartitionSQLVertexGenerator) Add(table schema.Table) (partialSQLGraph, error) {
	return buildPartialSQLGraph(
		buildAttachPartitionVertexId(table, diffTypeAddAlter),
		sqlPrioritySooner,
		a.addStatements(table),
		a.getAddDependencies(table),
	), nil
}

func (*attachPartitionSQLVertexGenerator) addStatements(table schema.Table) []Statement {
	if table.ParentTable == nil {
		return nil
	}

	return []Statement{{
		DDL: fmt.Sprintf("%s ATTACH PARTITION %s %s", alterTablePrefix(*table.ParentTable),
			table.GetFQEscapedName(), table.ForValues),
	}}
}

func (a *attachPartitionSQLVertexGenerator) Alter(diff tableDiff) (partialSQLGraph, error) {
	return buildPartialSQLGraph(
		buildAttachPartitionVertexId(diff.new, diffTypeAddAlter),
		sqlPrioritySooner,
		nil,
		nil,
	), nil
...
func (a *attachPartitionSQLVertexGenerator) Delete(table schema.Table) (partialSQLGraph, error) {
	return buildPartialSQLGraph(
		buildAttachPartitionVertexId(table, diffTypeDelete),
		sqlPriorityLater,
		nil,
		nil,
	), nil
...
func buildAttachPartitionVertexId(table schema.Table, diffType diffType) sqlVertexId {
	return buildSchemaObjVertexId("attachpartition", table.GetName(), diffType)
}

func (a *attachPartitionSQLVertexGenerator) getAddDependencies(table schema.Table) []dependency {
	if table.ParentTable == nil {
		return nil
	}

	deps := []dependency{
		mustRun(buildAttachPartitionVertexId(table, diffTypeAddAlter)).after(
			buildTableVertexId(table.SchemaQualifiedName, diffTypeAddAlter),
		),
	}
	...
	if _, baseTableIsNew := a.addedTablesByName[table.ParentTable.GetName()]; baseTableIsNew {
		...
		for _, idx := range a.indexesInNewSchemaByTableName[table.ParentTable.GetName()] {
			deps = append(deps, mustRun(buildAttachPartitionVertexId(table, diffTypeAddAlter)).before(
				buildIndexVertexId(idx.GetSchemaQualifiedName(), diffTypeAddAlter),
			))
		}
		return deps
	}
	...
	for _, idx := range a.indexesInNewSchemaByTableName[table.GetName()] {
		deps = append(deps, mustRun(buildAttachPartitionVertexId(table, diffTypeAddAlter)).after(
			buildIndexVertexId(idx.GetSchemaQualifiedName(), diffTypeAddAlter),
		))
	}
	return deps
...
func buildForeignKeyConstraintDiff(fsg *foreignKeyConstraintSQLVertexGenerator,
	addedTablesByName map[string]schema.Table,
	old, new schema.ForeignKeyConstraint,
) (foreignKeyConstraintDiff, bool, error) {
	if _, isOnNewTable := addedTablesByName[new.OwningTable.GetName()]; isOnNewTable {
	...
	if _, err := fsg.alterStatements(fkDiff); err != nil {
		if errors.Is(err, ErrNotImplemented) {
			// The foreign key must be re-created if the diff cannot be reconciled via alter statements
			return foreignKeyConstraintDiff{}, true, nil
		}
		return foreignKeyConstraintDiff{}, false,
			fmt.Errorf("generating foreign key alter statements: %w", err)
	}
...
func newForeignKeyConstraintSQLVertexGenerator(oldAndNewSchema oldAndNew[schema.Schema],
	tableDiffs listDiff[schema.Table, tableDiff],
) *foreignKeyConstraintSQLVertexGenerator {
...
func (f *foreignKeyConstraintSQLVertexGenerator) Add(con schema.ForeignKeyConstraint) (partialSQLGraph, error) {
	statements, err := f.addStatements(con)
	if err != nil {
		return partialSQLGraph{}, fmt.Errorf("generating sql: %w", err)
	}
	return buildPartialSQLGraph(
		buildForeignKeyConstraintVertexId(con, diffTypeAddAlter),
		sqlPrioritySooner,
		statements,
		f.getAddAlterDependencies(con),
	), nil
}

func (f *foreignKeyConstraintSQLVertexGenerator) addStatements(con schema.ForeignKeyConstraint) ([]Statement, error) {
	if con.IsValid {
...
func (f *foreignKeyConstraintSQLVertexGenerator) addAsInvalidThenValidateStatements(con schema.ForeignKeyConstraint) []Statement {
	...
	return []Statement{
		{
			DDL: fmt.Sprintf("%s %s NOT VALID",
				addConstraintPrefix(con.OwningTable, con.EscapedName), con.ConstraintDef),
		},
		validateConstraintStatement(con.OwningTable, con.EscapedName),
	}
}

func (f *foreignKeyConstraintSQLVertexGenerator) Delete(con schema.ForeignKeyConstraint) (partialSQLGraph, error) {
	return buildPartialSQLGraph(
		buildForeignKeyConstraintVertexId(con, diffTypeDelete),
		sqlPriorityLater,
		f.deleteStatements(con),
		f.getDeleteDependencies(con),
	), nil
}

func (f *foreignKeyConstraintSQLVertexGenerator) deleteStatements(con schema.ForeignKeyConstraint) []Statement {
...
func (f *foreignKeyConstraintSQLVertexGenerator) Alter(diff foreignKeyConstraintDiff) (partialSQLGraph, error) {
	statements, err := f.alterStatements(diff)
	if err != nil {
		return partialSQLGraph{}, fmt.Errorf("generating sql: %w", err)
	}
	return buildPartialSQLGraph(
		buildForeignKeyConstraintVertexId(diff.new, diffTypeAddAlter),
		sqlPrioritySooner,
		statements,
		f.getAddAlterDependencies(diff.new),
	), nil
}

func (f *foreignKeyConstraintSQLVertexGenerator) alterStatements(diff foreignKeyConstraintDiff) ([]Statement, error) {
	var stmts []Statement
	if !diff.old.IsValid && diff.new.IsValid {
	...
	if !cmp.Equal(diff.old, diff.new) {
		return nil, fmt.Errorf("altering foreign key constraint to resolve the following diff %s: %w",
			cmp.Diff(diff.old, diff.new), ErrNotImplemented)
	}

	return stmts, nil
}

func buildForeignKeyConstraintVertexId(con schema.ForeignKeyConstraint, diffType diffType) sqlVertexId {
	return buildSchemaObjVertexId("fkconstraint", con.GetName(), diffType)
}

func (f *foreignKeyConstraintSQLVertexGenerator) getAddAlterDependencies(con schema.ForeignKeyConstraint) []dependency {
	addAlterVertexID := buildForeignKeyConstraintVertexId(con, diffTypeAddAlter)
	deps := []dependency{
		mustRun(addAlterVertexID).after(buildForeignKeyConstraintVertexId(con, diffTypeDelete)),
		mustRun(addAlterVertexID).after(
			buildTableVertexId(con.OwningTable, diffTypeAddAlter),
		),
		mustRun(addAlterVertexID).after(
			buildTableVertexId(con.ForeignTable, diffTypeAddAlter),
		),
	}
	...
	for _, i := range f.indexesInNewSchemaByTableName[con.ForeignTable.GetName()] {
		deps = append(deps, mustRun(addAlterVertexID).after(
			buildIndexVertexId(i.GetSchemaQualifiedName(), diffTypeAddAlter),
		))
		...
		for _, c := range f.childrenInNewSchemaByPartitionedIndexName[i.GetName()] {
			deps = append(deps, mustRun(addAlterVertexID).after(
				buildIndexVertexId(c.GetSchemaQualifiedName(), diffTypeAddAlter),
			))
		}
	}
...
func (f *foreignKeyConstraintSQLVertexGenerator) getDeleteDependencies(con schema.ForeignKeyConstraint) []dependency {
	deleteVertexID := buildForeignKeyConstraintVertexId(con, diffTypeDelete)
	deps := []dependency{
		mustRun(deleteVertexID).before(
			buildTableVertexId(con.OwningTable, diffTypeDelete),
		),
		mustRun(deleteVertexID).before(
			buildTableVertexId(con.ForeignTable, diffTypeDelete),
		),
	}
	...
	for _, i := range f.indexInOldSchemaByTableName[con.ForeignTable.GetName()] {
		deps = append(deps, mustRun(deleteVertexID).before(
			buildIndexVertexId(i.GetSchemaQualifiedName(), diffTypeDelete),
		))
		// Build a dependency on any child index if the index is partitioned
		for _, c := range f.childrenInOldSchemaByPartitionedIndexName[i.GetName()] {
			deps = append(deps, mustRun(deleteVertexID).before(
				buildIndexVertexId(c.GetSchemaQualifiedName(), diffTypeDelete),
			))
		}
	}
	return deps
...
func (s *sequenceSQLVertexGenerator) Add(seq schema.Sequence) (partialSQLGraph, error) {
	return buildPartialSQLGraph(
		buildSequenceVertexId(seq.SchemaQualifiedName, diffTypeAddAlter),
		sqlPrioritySooner,
		s.addStatements(seq),
		s.getAddAlterDependencies(seq),
	), nil
}

func (s *sequenceSQLVertexGenerator) addStatements(seq schema.Sequence) []Statement {
	return []Statement{
		s.buildAddAlterSequenceStatement(seq, false),
	}
}

func (s *sequenceSQLVertexGenerator) Delete(seq schema.Sequence) (partialSQLGraph, error) {
	return buildPartialSQLGraph(
		buildSequenceVertexId(seq.SchemaQualifiedName, diffTypeDelete),
		sqlPriorityLater,
		s.deleteStatements(seq),
		s.getDeleteDependencies(seq),
	), nil
}

func (s *sequenceSQLVertexGenerator) deleteStatements(seq schema.Sequence) []Statement {
	hazards := []MigrationHazard{{
		Type:    MigrationHazardTypeDeletesData,
...
func (s *sequenceSQLVertexGenerator) Alter(diff sequenceDiff) (partialSQLGraph, error) {
	statements, err := s.alterStatements(diff)
	if err != nil {
		return partialSQLGraph{}, fmt.Errorf("generating sql: %w", err)
	}
	return buildPartialSQLGraph(
		buildSequenceVertexId(diff.new.SchemaQualifiedName, diffTypeAddAlter),
		sqlPrioritySooner,
		statements,
		s.getAddAlterDependencies(diff.new),
	), nil
}

func (s *sequenceSQLVertexGenerator) alterStatements(diff sequenceDiff) ([]Statement, error) {
	var stmts []Statement
	...
	if !cmp.Equal(diff.old, diff.new) {
		return nil, fmt.Errorf("altering sequence to resolve the following diff %s: %w",
			cmp.Diff(diff.old, diff.new), ErrNotImplemented)
	}
...
func (s *sequenceSQLVertexGenerator) buildAddAlterSequenceStatement(seq schema.Sequence, isAlter bool) Statement {
	...
	fmt.Fprintf(&sb, "\tMINVALUE %d MAXVALUE %d\n", seq.MinValue, seq.MaxValue)
	cycleModifier := ""
	if !seq.Cycle {
		cycleModifier = "NO "
	}
	fmt.Fprintf(&sb, "\tSTART WITH %d CACHE %d %sCYCLE\n", seq.StartValue, seq.CacheSize, cycleModifier)
...
func (s *sequenceSQLVertexGenerator) getAddAlterDependencies(new schema.Sequence) []dependency {
	addAlterVertexID := buildSequenceVertexId(new.SchemaQualifiedName, diffTypeAddAlter)
	deps := []dependency{
		mustRun(addAlterVertexID).after(buildSequenceVertexId(new.SchemaQualifiedName, diffTypeDelete)),
	}
	if new.Owner != nil {
		// Sequences should be added/altered before the table they are owned by
		deps = append(deps, mustRun(addAlterVertexID).before(
			buildTableVertexId(new.Owner.TableName, diffTypeAddAlter),
		))
	}
	return deps
}
...
func (s *sequenceSQLVertexGenerator) getDeleteDependencies(seq schema.Sequence) []dependency {
	...
	if seq.Owner != nil {
		deps = append(deps, mustRun(buildSequenceVertexId(seq.SchemaQualifiedName, diffTypeDelete)).after(
			buildTableVertexId(seq.Owner.TableName, diffTypeDelete),
		))
	}
	return deps
}
...
func (s sequenceOwnershipSQLVertexGenerator) Add(seq schema.Sequence) (partialSQLGraph, error) {
	return buildPartialSQLGraph(
		buildSequenceOwnershipVertexId(seq, diffTypeAddAlter),
		sqlPrioritySooner,
		s.addStatements(seq),
		s.getAddDependencies(seq),
	), nil
}

func (s sequenceOwnershipSQLVertexGenerator) addStatements(seq schema.Sequence) []Statement {
	if seq.Owner == nil {
		// If a new sequence has no owner, we don't need to alter it. The default is no owner
		return nil
	}
	return []Statement{s.buildAlterOwnershipStmt(seq)}
}

func (s sequenceOwnershipSQLVertexGenerator) Delete(seq schema.Sequence) (partialSQLGraph, error) {
	return buildPartialSQLGraph(
		buildSequenceOwnershipVertexId(seq, diffTypeDelete),
		sqlPriorityLater,
		nil,
		nil,
	), nil
...
func (s sequenceOwnershipSQLVertexGenerator) Alter(diff sequenceDiff) (partialSQLGraph, error) {
	return buildPartialSQLGraph(
		buildSequenceOwnershipVertexId(diff.new, diffTypeAddAlter),
		sqlPrioritySooner,
		s.alterStatements(diff),
		s.getAlterDependencies(diff.new, diff.old),
	), nil
}

func (s sequenceOwnershipSQLVertexGenerator) alterStatements(diff sequenceDiff) []Statement {
	if cmp.Equal(diff.new.Owner, diff.old.Owner) {
		return nil
	}
	return []Statement{s.buildAlterOwnershipStmt(diff.new)}
}
...
func buildSequenceOwnershipVertexId(seq schema.Sequence, diffType diffType) sqlVertexId {
	return buildSchemaObjVertexId("sequence_ownership", seq.GetFQEscapedName(), diffType)
}

func (s sequenceOwnershipSQLVertexGenerator) getAddDependencies(new schema.Sequence) []dependency {
	if new.Owner == nil {
		return nil
	}

	addAlterVertexID := buildSequenceOwnershipVertexId(new, diffTypeAddAlter)
	return []dependency{
		mustRun(addAlterVertexID).after(
			buildSequenceVertexId(new.SchemaQualifiedName, diffTypeAddAlter),
		),
		mustRun(addAlterVertexID).after(
			buildTableVertexId(new.Owner.TableName, diffTypeAddAlter),
		),
...
func (s sequenceOwnershipSQLVertexGenerator) getAlterDependencies(new, old schema.Sequence) []dependency {
	...
	deps := []dependency{
		...
		mustRun(buildSequenceOwnershipVertexId(new, diffTypeAddAlter)).after(
			buildSequenceVertexId(new.SchemaQualifiedName, diffTypeAddAlter),
		),
	}
	...
	if old.Owner != nil {
		...
		deps = append(deps, mustRun(buildSequenceOwnershipVertexId(new, diffTypeAddAlter)).before(
			buildTableVertexId(old.Owner.TableName, diffTypeDelete),
		))
	}

	if new.Owner != nil {
		// Always update ownership after the new owner has been created
		deps = append(deps, mustRun(buildSequenceOwnershipVertexId(new, diffTypeAddAlter)).after(
			buildTableVertexId(new.Owner.TableName, diffTypeAddAlter),
		))
	}
...
func buildColumnDefinition(column schema.Column) (string, error) {
	sb := strings.Builder{}
	fmt.Fprintf(&sb, "%s %s", schema.EscapeIdentifier(column.Name), column.Type)
	if column.IsCollated() {
		fmt.Fprintf(&sb, " COLLATE %s", column.Collation.GetFQEscapedName())
	}
	if column.IsGenerated {
		fmt.Fprintf(&sb, " GENERATED ALWAYS AS (%s) STORED", column.GenerationExpression)
	} else if len(column.Default) > 0 {
		fmt.Fprintf(&sb, " DEFAULT %s", column.Default)
	}
...
func buildColumnIdentityDefinition(identity schema.ColumnIdentity) (string, error) {
	...
	return fmt.Sprintf("GENERATED %s AS IDENTITY (INCREMENT BY %d MINVALUE %d MAXVALUE %d START WITH %d CACHE %d %sCYCLE)",
		typeModifier, identity.Increment, identity.MinValue, identity.MaxValue,
		identity.StartValue, identity.CacheSize, cycleModifier), nil
}
