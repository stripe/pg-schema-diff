...
func (t *triggerSQLVertexGenerator) GetAddAlterDependencies(newTrigger, oldTrigger schema.Trigger) ([]dependency, error) {
	...
	deps := []dependency{
		mustRun(t.GetSQLVertexId(newTrigger, diffTypeAddAlter)).after(
			buildFunctionVertexId(newTrigger.Function, diffTypeAddAlter),
		),
		mustRun(t.GetSQLVertexId(newTrigger, diffTypeAddAlter)).after(
			buildTableVertexId(newTrigger.OwningTable, diffTypeAddAlter),
		),
	}
	...
	if !cmp.Equal(oldTrigger, schema.Trigger{}) {
		...
		deps = append(
			deps,
			mustRun(t.GetSQLVertexId(newTrigger, diffTypeAddAlter)).before(
...
func (t *triggerSQLVertexGenerator) GetDeleteDependencies(trigger schema.Trigger) ([]dependency, error) {
	return []dependency{
		mustRun(t.GetSQLVertexId(trigger, diffTypeDelete)).before(
