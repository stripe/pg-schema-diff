...
const getCatalogACLGrants = `-- name: GetCatalogACLGrants :many
WITH raw_acl AS (
    SELECT
...
type GetCatalogACLGrantsRow struct {
	ObjectClass      string
	ClassID          int64
	ObjectID         int64
	SubObjectID      int32
...
func (q *Queries) GetCatalogACLGrants(ctx context.Context, db DBTX) ([]GetCatalogACLGrantsRow, error) {
	rows, err := db.Query(ctx, getCatalogACLGrants)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetCatalogACLGrantsRow
	for rows.Next() {
		var i GetCatalogACLGrantsRow
		if err := rows.Scan(
			&i.ObjectClass,
			&i.ClassID,
			&i.ObjectID,
...
const getCatalogDefaultACLs = `-- name: GetCatalogDefaultACLs :many
SELECT
...
type GetCatalogDefaultACLsRow struct {
	DefaultAclOid   int64
	OwnerOid        int64
	OwnerName       string
	SchemaOid       int64
...
func (q *Queries) GetCatalogDefaultACLs(ctx context.Context, db DBTX) ([]GetCatalogDefaultACLsRow, error) {
	rows, err := db.Query(ctx, getCatalogDefaultACLs)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetCatalogDefaultACLsRow
	for rows.Next() {
		var i GetCatalogDefaultACLsRow
		if err := rows.Scan(
			&i.DefaultAclOid,
			&i.OwnerOid,
			&i.OwnerName,
...
const getCatalogRoleMemberships = `-- name: GetCatalogRoleMemberships :many
SELECT
...
type GetCatalogRoleMembershipsRow struct {
	RoleOid       int64
	RoleName      string
	MemberOid     int64
	MemberName    string
...
func (q *Queries) GetCatalogRoleMemberships(ctx context.Context, db DBTX) ([]GetCatalogRoleMembershipsRow, error) {
	rows, err := db.Query(ctx, getCatalogRoleMemberships)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetCatalogRoleMembershipsRow
	for rows.Next() {
		var i GetCatalogRoleMembershipsRow
		if err := rows.Scan(
			&i.RoleOid,
			&i.RoleName,
			&i.MemberOid,
...
const getCatalogRoles = `-- name: GetCatalogRoles :many
SELECT
...
type GetCatalogRolesRow struct {
	RoleOid            int64
	RoleName           string
	IsSuperuser        bool
...
func (q *Queries) GetCatalogRoles(ctx context.Context, db DBTX) ([]GetCatalogRolesRow, error) {
	rows, err := db.Query(ctx, getCatalogRoles)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetCatalogRolesRow
	for rows.Next() {
		var i GetCatalogRolesRow
		if err := rows.Scan(
			&i.RoleOid,
			&i.RoleName,
			&i.IsSuperuser,
			&i.InheritsPrivileges,
			&i.CanCreateRoles,
			&i.CanLogin,
			&i.CanReplicate,
			&i.BypassesRls,
